...
export function TeamsLeaderboardScreen({ navigation }: Props) {
  ...
  const { entries, isLoading, error, hasMore, loadMore, refetch } = useTeamsLeaderboard(
    sortBy,
    period,
  );
  ...
  const getSortValue = useCallback(
    (entry: TeamLeaderboardEntry) => {
      switch (sortBy) {
        case 'distance':
          return formatDist(entry.total_distance);
        case 'duration':
          return formatTotalTime(entry.total_duration);
        case 'activities':
          return `${entry.activities_count}`;
        case 'members':
          return `${entry.members_count}`;
        case 'elevation':
          return `${entry.total_elevation.toLocaleString()} m`;
        default:
          return formatDist(entry.total_distance);
      }
    },
    ...
    ({ item }: { item: TeamLeaderboardEntry }) => {
      ...
      return (
        ...
        >
          ...
          <Text style={[styles.sortValue, { color: colors.primary }]}>{getSortValue(item)}</Text>
        </TouchableOpacity>
      );
    },
  ...
  return (
    <ScreenContainer>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          !isLoading ? (
            <EmptyState icon="trophy-outline" title={t('teams.noLeaderboardEntries')} />
          ) : null
        }
        ListFooterComponent={
          isLoading ? (
