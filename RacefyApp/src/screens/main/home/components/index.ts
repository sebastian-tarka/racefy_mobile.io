export { ConnectionErrorBanner } from './ConnectionErrorBanner';
export { HomeHeader } from './HomeHeader';
export { HeroSection } from './HeroSection';
export { AuthCard } from './AuthCard';
export { FeaturesList } from './FeaturesList';
export { StatsSection } from './StatsSection';
export { UpcomingEventsPreview } from './UpcomingEventsPreview';
export { OngoingEventsPreview } from './OngoingEventsPreview';
export { ActivitiesFeedPreview } from './ActivitiesFeedPreview';
export { DynamicGreeting } from './DynamicGreeting';
export { WeeklyStatsCard } from './WeeklyStatsCard';
export { WeeklyStatsCardV2 } from './WeeklyStatsCardV2';
export { QuickActionsBar } from './QuickActionsBar';
export { QuickActionsBarV2 } from './QuickActionsBarV2';
export { LiveActivityBanner } from './LiveActivityBanner';
export { WeeklyStreakCard } from './WeeklyStreakCard';
export { StatCard } from './StatCard';
export { CollapsibleTipCard } from './CollapsibleTipCard';
export { LiveEventsCard } from './LiveEventsCard';

// Dynamic Home Config components
export { PrimaryCTA } from './PrimaryCTA';
export { SectionRenderer, getSectionComponentName } from './SectionRenderer';
export * from './sections';
