...
export function useHealthEnrichment() {
  const enrichActivityWithHeartRate = useCallback(async (activity: Activity): Promise<Activity> => {
    try {
      ...
      const endTime = activity.ended_at ? new Date(activity.ended_at) : new Date();
