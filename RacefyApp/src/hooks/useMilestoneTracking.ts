 ...
 */
export function useMilestoneTracking(distance: number, milestones: MilestoneSingle[]) {
  const [passedMilestones, setPassedMilestones] = useState<number[]>([]);

  // Check milestones during activity recording
  useEffect(() => {
    milestones.forEach((milestone) => {
      if (distance >= milestone.threshold && !passedMilestones.includes(milestone.threshold)) {
